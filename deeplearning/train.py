...
def train(model, train_loader, valid_loader, loss_fn, optimizer, lr_scheduler, model_name, device):
    ...
    for epoch in range(7):
        ...
        for batch_idx, (input_ids, token_type_ids, attention_mask, y) in loop:
            ...
            prediction = model(input_ids, token_type_ids=token_type_ids, attention_mask=attention_mask)
        ...
        with torch.no_grad():
           for batch_idx, (input_ids, token_type_ids, attention_mask, y) in enumerate(valid_loader):
                optimizer.zero_grad()
        ...
        if val_acc > best_acc: 
            torch.save(model, '/content/drive/MyDrive/MSC/DeepLearning/HW4/Q1/checkpoints/' + model_name)
            best_acc = val_acc 
